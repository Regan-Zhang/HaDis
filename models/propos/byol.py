...
class BYOL(TrainTask):
    ...
    def train(self, inputs, indices, n_iter):
        ...
        with torch.autocast('cuda', enabled=opt.amp):
            contrastive_loss, cluster_loss_batch, hsd_loss, q = self.byol(
                im_q, im_k, indices, update_params, opt.v2)

        loss = contrastive_loss + hsd_loss + cluster_loss_batch * opt.cluster_loss_weight * float(not is_warmup)

        with open(f"{self.logger.log_save_dir}/loss_logger.txt", "a") as f:
            f.write(
                f"Epoch[{self.cur_epoch}] contrastive_loss={contrastive_loss}, cluster_loss_batch={cluster_loss_batch}, hsd_loss={hsd_loss}\n")
