...
class BYOLWrapper(MoCoWrapper):
    ...
    def forward_loss(self, im_q, im_k, psedo_labels: torch.Tensor):
        ...
        # Add HSD loss
        hsd_loss = self.hsd(k, self.predictor(noise_q))
        ...
        return contrastive_loss, cluster_loss_batch, hsd_loss, all_q, all_k
    ...
    def forward(self, im_q, im_k, indices, momentum_update=True, v2=False):
        ...
        if self.symmetric:
            contrastive_loss1, cluster_loss_batch1, hsd_loss_1, q1, k1 = self.forward_loss(im_q, im_k, psedo_labels)
            contrastive_loss2, cluster_loss_batch2, hsd_loss_2, q2, k2 = self.forward_loss(im_k, im_q, psedo_labels)
            contrastive_loss = 0.5 * (contrastive_loss1 + contrastive_loss2)
            cluster_loss_batch = 0.5 * (cluster_loss_batch1 + cluster_loss_batch2)
            hsd_loss = 0.5 * (hsd_loss_1 + hsd_loss_2)
            q = torch.cat([q1, q2], dim=0)
            k = torch.cat([k1, k2], dim=0)
        else:  # asymmetric loss
            contrastive_loss, cluster_loss_batch, hsd_loss, q, k = self.forward_loss(im_q, im_k, psedo_labels)
        ...
        return contrastive_loss, cluster_loss_batch, hsd_loss, q
